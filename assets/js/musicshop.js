...
itemsContainer.addEventListener('scroll',()=>{
    if(itemsContainer.scrollTop + itemsContainer.clientHeight > 1.1*itemsContainer.clientHeight && itemsContainerHeightGrow == false){
      grow_itemsContainer(true);
    }
    else if(itemsContainer.scrollTop == 0 && itemsContainerHeightGrow == true){
      grow_itemsContainer(false);
    }
});
itemsContainerHeight = parseInt(getComputedStyle(itemsContainer).height.replace('px',''));
displayItemsContainerHeight = parseInt(getComputedStyle(displayItemsContainer).height.replace('px',''));



function grow_itemsContainer(growValue){
 itemsContainerHeight = parseInt(getComputedStyle(itemsContainer).maxHeight.replace('px',''));
if(growValue){
      timing = setInterval(()=>{
       if(itemsContainerHeight < MaxGrowHeight){
       itemsContainerHeight = itemsContainerHeight + 10;
       itemsContainer.style.maxHeight = itemsContainerHeight + 'px';
       displayItemsContainer.style.maxHeight = MaxGrowHeight - itemsContainerHeight;
       itemsContainerHeightGrow = true;

      }
      else {
        timing = clearInterval();
      }
      ...
      timing = setInterval(()=>{
       if(itemsContainerHeight > StandardGrowHeight){
       itemsContainerHeight = itemsContainerHeight - 10;
       itemsContainer.style.maxHeight = itemsContainerHeight + 'px';
       displayItemsContainer.style.maxHeight = MaxGrowHeight - itemsContainerHeight;
        itemsContainerHeightGrow = false;
      }
      else {
        timing = clearInterval();
      }
